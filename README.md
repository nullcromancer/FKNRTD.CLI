...
*The wide dashboard, rendered by the program itself. Every frame in this README is generated by
`scripts/capture-frames.py`, so none of them can drift from what the product actually draws.*
...
(`tests/FKNRTD.SelfTest/Program.cs`), currently 38 checks, all passing on this checkout.
...
Nothing below is a mock-up. Every frame is the program's own renderer, captured by
`scripts/capture-frames.py`, which asks the self-test harness for a named scene and turns the ANSI
it prints into an image. The captures are generated for the same reason the operator guide is: a
screenshot committed once is free to go on showing a keymap the product no longer has.
...
![The help and glossary browser](docs/screenshots/help.png)

`/` opens the command palette. Actions you cannot take right now are listed anyway, with the
reason — not hidden, and not silently inert.

![The command palette](docs/screenshots/commands.png)

`I` opens a task's full record: the brief, who holds which role and what that role may touch, where
its worktree is, what decides correctness, every stage with what that stage is for, and what to do
next.
...
you to have read the diff; this is the view that lets you.

![The finished change](docs/screenshots/diff.png)

Anything irreversible states what it is about to do, and where to read the diff first, before
asking for the typed confirmation it still requires.

![The landing confirmation](docs/screenshots/land.png)

`S` explains every setting in `.fknrtd/config.json` — with the value this workspace is actually
running, and what changing it would cost.

![The settings browser](docs/screenshots/settings.png)

A workspace with no tasks opens on an introduction rather than an empty grid.

![The first-run introduction](docs/screenshots/welcome.png)

| Medium, 100 columns | Narrow, 74 columns |
| --- | --- |
...
when every check passes. Verified on this checkout: `38/38 self-tests passed`.
